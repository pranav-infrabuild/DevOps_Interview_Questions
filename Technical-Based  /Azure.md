...
## 🔁 🔹 Q20. What is Azure Logic Apps?

<details>

Azure Logic Apps is a cloud-based workflow automation service from Microsoft that lets you automate business processes and integrate different services without writing custom code.

Think of it as a smart coordinator — when something happens in one system, Logic Apps automatically takes action in one or more other systems.
...
### Real-World Use Case
Automate approvals, notifications, integrations, data synchronization, and workflow orchestration.

### Interview Answer

"Azure Logic Apps is a cloud-based workflow automation service from Microsoft that lets you automate business processes and integrate different services without writing custom code. You define a workflow visually — a trigger that starts it, and a series of actions that follow. It connects to over 400 services out of the box, including Office 365, SQL, Teams, Salesforce, and SAP. Think of it as a smart coordinator — when something happens in one system, Logic Apps automatically takes action in one or more other systems."

</details>

---

## ⚡ 🔹 Q21. What is the difference between a Trigger and an Action in Logic Apps?

<details>

Logic Apps workflows are built using triggers and actions.
...
"A trigger is the event that starts a Logic App workflow — it could be an HTTP request coming in, a new email arriving, a file being uploaded, or a scheduled time. Every Logic App must have exactly one trigger. An action is what the workflow does after being triggered — sending an email, inserting a database record, posting a Teams message, or calling an API. A workflow can have as many actions as needed. Simply put: the trigger is the 'when', and the actions are the 'what'."

</details>

---

## 🔔 🔹 Q22. What types of triggers are available in Logic Apps?

<details>

Logic Apps supports multiple trigger types depending on integration needs.

### 1. HTTP Trigger
Starts when an external system sends an HTTP request.
...
### 2. Polling Trigger
Checks a source periodically for updates.
...
### 3. Push Trigger
External service notifies Logic Apps immediately.
...
"There are three main types of triggers. First, HTTP triggers — the workflow starts when an external system sends an HTTP request to a generated URL, which is what I used in my expense approval project. Second, polling triggers — Logic Apps periodically checks a source for new data, like checking an email inbox every 5 minutes. Third, push triggers — an external service notifies Logic Apps when an event happens, like a new message arriving in a Service Bus queue. The right trigger depends on the use case — HTTP for real-time on-demand, polling for periodic checks, and push for event-driven integrations."

</details>

---

## 🔌 🔹 Q23. What is a Connector in Logic Apps?

<details>

A connector is a pre-built integration component that allows Logic Apps to communicate with external systems.
...
#### Built-in Connectors
Run directly inside Logic Apps engine.
...
#### Managed Connectors
Hosted by Microsoft with built-in authentication support.
...
"A connector is a pre-built integration component that allows Logic Apps to communicate with a specific service or system. For example, the Office 365 connector handles sending and receiving emails, the SQL Server connector reads and writes database records, and the Teams connector posts messages to channels. There are two types — built-in connectors like HTTP and Schedule that run directly in the Logic Apps engine, and managed connectors like Salesforce or SAP that are hosted by Microsoft and include authentication handling. With over 400 connectors available, most integrations require zero custom code."

</details>

---

## 💰 🔹 Q24. What are the pricing plans in Logic Apps?

<details>

Logic Apps offers two major pricing models.
...
"Logic Apps offers two main plans. The Consumption plan charges per action execution — you only pay when the workflow actually runs. It's multi-tenant, quick to set up, and ideal for scenarios with moderate or unpredictable traffic. This is what I used for the expense approval workflow since it runs only when employees submit requests. The Standard plan runs on a dedicated single-tenant environment with a fixed monthly cost. It supports VNet integration, private networking, and is suited for enterprise workloads that need isolation, higher performance, or compliance requirements. For most automation projects, Consumption is the right starting point."
...
## 🧾 🔹 Q25. Walk me through a real Logic Apps project you built.

<details>

Example project: Employee Expense Approval Workflow

### Problem
Manual approvals took:
- 3 to 4 days
- No visibility
- Manual follow-ups

### Solution Workflow
1. HTTP trigger from web form
2. Check if amount > ₹5,000
...
### Result
- Approval reduced to under 15 minutes
- Full audit trail
- Zero custom code

### Interview Answer

"I built an employee expense approval system. The problem was that the manual process took 3 to 4 days with no visibility for anyone involved. I created a Logic App triggered by an HTTP request from a web form. The workflow first checks if the amount exceeds ₹5,000 — if yes, it sends an approval email to the manager via Office 365 with Approve and Reject buttons embedded. The Logic App then pauses and waits for the manager's response. Based on the decision, it either notifies the finance team and inserts an approved record into Azure SQL, or sends the employee a rejection email. Finally, it posts a message to the Finance channel in Microsoft Teams. Approval turnaround dropped from 3–4 days to under 15 minutes, with zero custom code and a full audit trail in SQL."

</details>

---

## ⏸️ 🔹 Q26. How does approval workflow pause for human response?

<details>

Logic Apps supports built-in approval workflows.
...
- Auto-escalate to higher manager

### Interview Answer

"Logic Apps has a built-in 'Send approval email' action in the Office 365 connector. When this action runs, it sends an email with selectable options — like Approve and Reject — and then the workflow pauses. Azure persists the workflow state automatically, so no server needs to keep running during the wait. When the manager clicks a button in the email, Logic Apps resumes from exactly where it paused and the selected option is available as a variable in subsequent steps. I also configured a timeout of 48 hours — if the manager doesn't respond within that window, the workflow takes a timeout branch and auto-escalates to the skip-level manager."

</details>

---

## 🔀 🔹 Q27. How do you implement conditional logic in Logic Apps?

<details>

Logic Apps provides workflow control statements.
...
"Logic Apps has a Condition control that works like an if-else statement. You define a condition — for example, checking if an amount field is greater than 5,000 — and the designer splits into a True branch and a False branch. Each branch can contain its own set of actions. You can also nest conditions inside each other for more complex logic. For more than two branches, there's a Switch control which works like a switch-case statement — you check a single value and route to different action blocks based on what it matches. In my expense project, I used two separate conditions — one for the amount threshold and one for the manager's approval decision."
